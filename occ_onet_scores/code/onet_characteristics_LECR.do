...
clear
cd"D:\Dropbox\GitHub\Felipe_Balcazar\Dingel_Neiman_project\DingelNeiman-workathome\occ_onet_scores" //we set the correct working directory

import delimited using "./input/ONET_Occupation_Data.txt", delim(tab) clear varnames(1)
tempfile tf_occtitles
save "tf_occtitles.dta", replace

//JOB CHARACTERISTICS: WORK ACTIVITIES
import delimited using "./input/ONET_Work_Activities.txt", delim(tab) clear
keep if inlist(elementid,"4.A.3.a.1","4.A.3.a.2","4.A.3.a.3","4.A.3.a.4","4.A.4.a.8","4.A.3.b.4","4.A.3.b.5","4.A.1.b.2") //keeps only if elementid is equal to one of this values
bys onetsoccode: egen byte physical_activities = max(elementid=="4.A.3.a.1" & scaleid=="IM" & inrange(datavalue,4.0,5.0)==1) //This command groups the data by the onetsoccode variable and creates a new variable physical_activities. For each group, if there's at least one observation where elementid is "4.A.3.a.1", scaleid is "IM", and datavalue is between 4.0 and 5.0 (inclusive), physical_activities is set to 1; otherwise, it's set to 0.
label variable physical_activities "Performing General Physical Activities is very important (4.0+ of 5)" //This command assigns a label to the physical_activities variable.
...
destring n, force replace //converting a string variable to numeric
collapse (firstnm) physical_activities handlingobjects control_machines operate_equipment dealwithpublic repair_mechequip repair_elecequip inspect_equip (mean) n, by(onetsoccode) //This command collapses the data into one observation per onetsoccode

tempfile tf_workactivities
save "tf_workactivities.dta", replace
...
inlist(elementid,"4.C.1.a.2.f","4.C.1.a.2.h","4.C.1.b.1.f","4.C.1.b.1.g","4.C.1.c.1","4.C.1.d.3","4.C.2.a.1.b","4.C.2.a.3")==1 | ///returns 1 if the value is in the list and 0 otherwise.
...
save "tf_workcontext.dta", replace

//Merge three files
use "tf_workcontext.dta", clear
merge 1:1 onetsoccode using "tf_workactivities.dta", assert(match) nogen
merge m:1 onetsoccode using "tf_occtitles.dta", assert(using match) keep(match) nogen keepusing(title)
...
//detele the temporal files
erase "tf_workcontext.dta"
erase "tf_workactivities.dta"
erase "tf_occtitles.dta"
