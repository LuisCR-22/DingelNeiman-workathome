...
} //this code checks if the "listtex" package is installed in the Stata environment, and if it's not, it installs it.
...
//assert: If the merge was not successful (i.e., if there are observations in the using dataset that do not match with observations in the master dataset), Stata will return an error.
//nogen: This option prevents Stata from automatically generating a variable (_merge)

rename Teleworkable teleworkable
//Generate employment-weighted and wage-weighted means
destring TOT_EMP, replace ig(*)
//converts the TOT_EMP variable from a string to a numeric variable. 
//ig(*): This option tells Stata to ignore any characters that cannot be converted to numbers. In this case, it's telling Stata to ignore all non-numeric characters (* is a wildcard that matches any character).
...
save "tf_forcsv_manual.dta", replace
...
//Generates rankings for the teleworkability employment variable and keeps only the rows where the rank is in the top 5 or bottom 5. ¿Why? -> to generate tables in LaTex.
...
erase "tf_forcsv_baseline.dta"
erase "tf_forcsv_manual.dta"
