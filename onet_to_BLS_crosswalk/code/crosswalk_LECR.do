...
cd "D:\Dropbox\GitHub\Felipe_Balcazar\Dingel_Neiman_project\DingelNeiman-workathome\onet_to_BLS_crosswalk"
//mkdir "./input"

tempfile tf_onet_soc tf_soc_bls tf_bls_telework //create temporary file names that can be used to store intermediate results.
...
save "tf_onet_soc.dta" //temporal file that I will erase
...
merge 1:1 onetsoccode using "tf_onet_soc.dta", assert(using match) keep(match) keepusing(SOC_2010) nogen
...
merge 1:m SOC_2010 using "tf_soc_bls.dta", keep(using match)
...
erase "tf_onet_soc.dta" 
erase "tf_soc_bls.dta"
